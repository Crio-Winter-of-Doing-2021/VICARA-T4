...
import {selectSearchResults,searchUserAsync,selectPatchUsers} from '../../store/slices/shareSlice'
...
export default function Users(){
    ...
    let userRenderer=results.map((user)=>{
        ...
        return(
         <UserStyleUtil show={show} data={user}/>
        )
    })
    ...
    return(
        <div style={{marginTop:"20px"}}>
            <TextField
                ...
                fullWidth
            />
            <Popper className={classes.root} open={open} anchorEl={anchorEl} placement='bottom' transition>
                {({ TransitionProps }) => (
                <Fade {...TransitionProps} timeout={350}>
                    <Paper style={{maxHeight:"300px",overflow:"scroll",borderRadius:"20px"}}>
                        <div>
                            {loading?
                            ...
                            <div style={{width:"100%"}}>
                                ...
                                <div style={{display:"flex",justifyContent:"center",alignItems:"center",height:"150px"}}>
                                    <Typography style={{color:"grey",fontStyle:"italic"}}>No results found.</Typography>
                                </div>:userRenderer}
                            </div>}
                        </div>
                    </Paper>
